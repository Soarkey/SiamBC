...
class SiamBCTracker(object):
    def __init__(self, info):
        super(SiamBCTracker, self).__init__()
    ...
    def init(self, im, target_pos, target_sz, model, hp=None):
        ...
        if not hp and not self.info.epoch_test:
            ...
            cfg_benchmark = config.TEST[prefix]
        ...
        if self.info.epoch_test:
            p.update(config.TEST[self.info.dataset])
            p.renew()

            if ((target_sz[0] * target_sz[1]) / float(state['im_h'] * state['im_w'])) < 0.004:
                p.instance_size = config.TEST[self.info.dataset]['big_sz']
                p.renew()
            else:
                p.instance_size = config.TEST[self.info.dataset]['small_sz']
                p.renew()
    ...
    def update(self, net, x_crops, target_pos, target_sz, window, scale_z, p):
        ...
        # size penalty
        s_c = self.change(self.sz(pred_x2 - pred_x1, pred_y2 - pred_y1) / (self.sz_wh(target_sz) + 1e-10))  # scale penalty
        r_c = self.change((target_sz[0] / (target_sz[1] + 1e-10)) / ((pred_x2 - pred_x1) / (pred_y2 - pred_y1 + 1e-10)) + 1e-10)  # ratio penalty
        ...
        pred_xs = (pred_x1 + pred_x2) / 2.0
        pred_ys = (pred_y1 + pred_y2) / 2.0
    ...
    def update_cam(self, net, x_crops, target_pos, target_sz, window, scale_z, p):
        if self.align:
            ori_cls_score, bbox_pred, cls_align = net.track(x_crops)

            cls_score = F.sigmoid(ori_cls_score).squeeze().cpu().data.numpy()
            cls_align = F.sigmoid(cls_align).squeeze().cpu().data.numpy()
            cls_score = p.ratio * cls_score + (1 - p.ratio) * cls_align
        else:
            ori_cls_score, bbox_pred = net.track(x_crops)
            cls_score = F.sigmoid(ori_cls_score).squeeze().cpu().data.numpy()

        cv2.imwrite(f"vis/cls/{self.index}.png", ori_cls_score.squeeze().permute(1, 2, 0).cpu().numpy())
        self.index += 1
