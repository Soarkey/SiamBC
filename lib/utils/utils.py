...
def load_dataset(dataset):
    ...
    if 'OTB' in dataset:
        ...
        for video in videos:
            # 兼容siamx测试LASOT的问题
            if video not in testingvideos or 'json' in video:  # 280 testing videos
                continue
...
def load_pretrain(model, pretrained_path, print_unuse=True):
    ...
    if pretrained_path.endswith('pkl'):
        # for pickle model file
        import pickle as pkl
        with open(pretrained_path, 'rb') as f:
            pretrained_dict = pkl.load(f)
    else:
...
def print_speed(i, i_time, n, logger):
    ...
    logger.info('Progress: %d / %d [%d%%], Speed: %.3f s/iter, ETA %d:%02d:%02d (D:H:M)' % (i, n, i / n * 100, average_time, remaining_day, remaining_hour, remaining_min))
...
def create_logger(cfg, phase='train'):
    ...
    return logger, str(root_output_dir), str(tensorboard_log_dir)
