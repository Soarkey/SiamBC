...
from lib.models.connect import AdjustLayer, AlignHead, box_tower


class SiamBC(nn.Module):
    def __init__(self, align=False):
        super(SiamBC, self).__init__()
        ...
        self.batch = config.TRAIN.BATCH
        self.grids()

        if config.TRAIN.BACKBONE == 'ResNet':
            self.features = ResNet50(used_layers=config.TRAIN.USED_LAYERS)  # in param
        else:
            raise NotImplementedError(config.TRAIN.BACKBONE, " is not supported!")

        hidden_channels = config.TRAIN.HIDDEN_CHANNELS  # default is 256

        self.neck = AdjustLayer(in_channels=config.TRAIN.IN_CHANNELS, out_channels=hidden_channels)
        if config.TRAIN.TOWER.TYPE == 'box_tower':
            self.connect_model = box_tower(inchannels=hidden_channels, outchannels=hidden_channels, towernum=4)
        elif config.TRAIN.TOWER.TYPE == 'cascade_tower':
            from lib.models.cascade import cascade_tower
            self.connect_model = cascade_tower(in_channels=hidden_channels, out_channels=hidden_channels)
        else:
            raise NotImplementedError(config.TRAIN.TOWER, " is illegal")
        self.align_head = AlignHead(hidden_channels, hidden_channels) if align else None
    ...
    def forward(self, template, search, label=None, reg_target=None, reg_weight=None):
        """
        Args:
            template: [32,3,127,127]
            search: [32,3,255,255]
            label: [32,25,25]
            reg_target: [32,25,25,4]
            reg_weight: [32,25,25]
        Returns:
        ...
        if self.align_head is not None:
            ...
            # 测试参数量时开启
            # cls_loss = self.criterion(cls_align.squeeze(), align_cls_label.squeeze())
