...
class TrackingDataset(Dataset):
    def __init__(self, cfg):
        ...
        self.gray = cfg.DATASET.GRAY
        self.label_smooth = cfg.DATASET.LABELSMOOTH
        self.mixup = cfg.DATASET.MIXUP
        self.cutout = cfg.DATASET.CUTOUT

        # aug for search image
        self.shift_s = cfg.DATASET.SHIFTs
        self.scale_s = cfg.DATASET.SCALEs

        self.grids()

        self.random = [random.random() for _ in range(5)]
        self.transform_extra = transforms.Compose(
            [transforms.ToPILImage(), ]
            + ([transforms.ColorJitter(0.05, 0.05, 0.05, 0.05), ] if self.color > self.random[0] else [])
            + ([transforms.RandomHorizontalFlip(p=1), ] if self.flip > self.random[1] else [])
            + ([transforms.RandomRotation(degrees=10), ] if self.rotation > self.random[2] else [])
            + ([transforms.Grayscale(num_output_channels=3), ] if self.gray > self.random[3] else [])
            + ([Cutout(n_holes=1, length=16)] if self.cutout > self.random[4] else [])
        )

        # train data information
        print('train datas: {}'.format(cfg.TRAIN.WHICH_USE))
        start = 0
        self.train_datas = []  # all train dataset
        self.num = 0
        for data_name in cfg.TRAIN.WHICH_USE:
        ...
        if cfg.TRAIN.BACKBONE == 'ConvNext':
            self.num *= cfg.TRAIN.END_EPOCH
        self._shuffle()
        print(cfg)
    ...
    def _draw(self, image, box, name):
        ...
        cv2.putText(draw_image, '[x: {}, y: {}]'.format(int(round(x1 + x2) / 2), int(round(y1 + y2) / 2)), (int(round(x1 + x2) / 2) - 3, int(round(y1 + y2) / 2) - 3), cv2.FONT_HERSHEY_SIMPLEX, 0.3,
                    (255, 255, 255), 1)
        # cv2.imwrite(name, draw_image)
        cv2.imshow(name, draw_image)
        cv2.waitKey(100)
    ...
    def _augmentation(self, image, bbox, size, search=False):
        ...
        # 翻转
        if self.flip > self.random[1]:
            width = image.shape[0]
            bbox = Corner(width - bbox.x2, bbox.y1, width - bbox.x1, bbox.y2)
    ...
    def _create_dynamic_logisticloss_label(self, label_size, c_shift, rPos=2, rNeg=0):
        ...
        if config.TRAIN.LABEL_ASSIGN:
            # label assignment
            label = np.where(dist_to_center <= rPos,
                             np.ones_like(y),
                             np.where(dist_to_center <= 2 * rPos,
                                      -np.ones_like(y),
                                      np.zeros_like(y)))
        else:
            label = np.where(dist_to_center <= rPos,
